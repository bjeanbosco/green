...
async function postHandler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    ...
    const newStaffMember = new StaffModel({ name, email, phoneNumber, department });
...
async function updateHandler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    ...
    const updatedStaff = await StaffModel.findByIdAndUpdate(id, { name, email, phoneNumber, department }, { new: true });
    if (updatedStaff) {
