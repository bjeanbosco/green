...
const LearnersClubs = ({ user }: any) => {
  ...
  const fetchSections = async () => {
    try {
      const response = await axios.get("/api/learnerClub");
  ...
  const handleUpdate = async (slug: string) => {
    ...
    try {
      ...
      await axios.put(`/api/learnerClub?slug=${slug}`, updatedContent);
      await fetchSections();
      setEditMode(false);
    } catch (error) {
  ...
  const handleDelete = async (slug: string) => {
    try {
      await axios.delete(`/api/learnerClub?slug=${slug}`);
      await fetchSections();
    } catch (error) {
  ...
  return (
    <main className="w-full">
      ...
      {sections.map((section: any, index: React.Key | null | undefined) => (
        <section key={index} className="w-full mb-16">
          {section.slug === "intellectual" && (
